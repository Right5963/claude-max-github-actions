...
## 💰 **最重要: プロジェクトの目的 (2025-06-04記録)** ⚡絶対に忘れるな

### **明確な事業目的**
画像生成でFANZA同人、DL同人、ヤフオクなどで可愛い美女やエロ漫画、エロイラストなどのアダルトを販売してお金を稼ぐ

### **なぜこのシステムを作っているのか**
- **AI開発ツール作成**: 効率的な画像生成のため
- **MCP活用**: 市場リサーチと情報収集の自動化
- **Obsidian活用**: 知識の蓄積と活用
- **全ては収益化のため**: これが最重要
...
どういった要素が売れるのかを徹底調査：
- **構図**: どんな構図が人気か、売れているか
- **美女**: どんな顔、髪型、体型、表情が需要あるか
- **ポーズ**: どんなポーズが売れ筋か
- **イラストスタイル**: どんな画風が人気か
- **文章**: 売れるタイトル、説明文、キーワード
- **ジャンル**: どのジャンルが売れているか

### **技術収集の目的**
Civitaiから収集するもの：
- **プロンプト**: 売れ筋画像を生成するための呪文
- **LoRA**: 特定のスタイル・キャラを再現
- **モデル**: 高品質な画像生成の基盤
- **全ては需要のあるイラスト生成のため**
...
### **記録の重要性**
- この目的を常に意識する
- 全ての開発・リサーチはこの収益化に向ける
- Obsidianに全ての知見を蓄積し活用する
- 二度と忘れないよう、常に参照する

**これが一番重要なこと。この目的から外れた作業は無意味。**

---

## 🚨 **新セッション開始プロトコル（必須実行）** ⚡例外なし

### **毎回最初に必ず実行**
```
□ このCLAUDE.md読み込み完了
□ python3 git_quick_insight.py 実行  
□ Gドライブアクセス確認（MCPブリッジ経由）
□ ./mcp_bridge_extended.sh obsidian_write 確認
□ 既存システム・自動起動ツール確認
...
```
保存時必須事項:
✓ 適切なフォルダ配置（AI生成収益化事業/XX_カテゴリ/）
✓ 階層タグ付け（#AI生成事業/技術/MCP等）
✓ 関連文書リンク構築
✓ なぜ重要かの明記
✓ 他知識との関連性記述

コマンド: ./mcp_bridge_extended.sh obsidian_write "パス" "内容"
```
...
## 🎯 **MCP開発成功パターン (2025-06-05記録)** ⚡最重要

### **成功事例: DMM Affiliate MCP vs FANZA Login MCP**

#### ✅ 成功パターン（DMM Affiliate MCP）
```typescript
// 公式API使用
const API_BASE_URL = "https://api.dmm.com/affiliate/v3";
const params = { api_id: API_ID, affiliate_id: AFFILIATE_ID, site: "FANZA", service: "doujin" };
```

**成功要因:**
- ✅ **公式API活用**: 安定性・セキュリティ・保守性
- ✅ **段階的実装**: MVP → 機能拡張  
- ✅ **適切なエラー処理**: try-catch + ログ出力
- ✅ **シンプル認証**: URLパラメータのみ
- ✅ **実用価値**: 50,000件の実データ取得

#### ❌ 失敗パターン（FANZA Login MCP）
```typescript
// ブラウザ自動化・DOM操作
const loginUrl = 'https://www.dmm.co.jp/service/login/password/authenticate';
// → CAPTCHA、CSRF、年齢認証の多段階処理
```

**失敗要因:**
- ❌ **ブラウザ自動化**: DOM変更・CAPTCHA・規約違反リスク
- ❌ **完璧主義**: 一度に全機能実装 → 複雑化
- ❌ **エラー処理軽視**: 404エラー → 実装継続
...
```bash
# 必須チェック
node verify_api_real.js  # 実際のデータ取得確認
```
- 架空データでの実装は絶対禁止
- 動作確認 = 実データ取得確認
- 「動く」と「使える」は別
...
#### 3. **段階的価値提供**
```
Phase 1: 基本機能（検索）→ 実用価値確認
Phase 2: 追加機能（ランキング）→ 価値拡張  
Phase 3: 高度機能（分析）→ 差別化
```

#### 4. **成功の方程式**
```
成功 = 公式API × シンプル実装 × 段階的開発 × 実用価値
失敗 = スクレイピング × 複雑実装 × 完璧主義 × 架空データ
```
